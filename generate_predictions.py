#!/usr/bin/env python3
"""
Hull Tactical Market Prediction - Generador de Predicciones
Genera archivo .parquet con predicciones para submission directa
...
def generate_predictions():
    ...
    print("🚀 Iniciando generación de predicciones Hull Tactical...")
    ...
    try:
        ...
        try:
            ...
            print("⚠️ Archivo test.csv no encontrado, usando últimos datos de train como test")
            test_df = train_df.tail(500).copy()
            test_df = test_df.drop('forward_return_1d', axis=1, errors='ignore')
        ...
        train_data['forward_return_1d'] = signal
        ...
        for i in range(n_features):
            if i < 10:
                trend = np.sin(np.arange(n_train, n_train + n_test) * 0.01) * 0.1
        ...
        print(f"📊 Datos sintéticos - Train: {train_df.shape}, Test: {test_df.shape}")
    ...
    y_train_full = train_clean['forward_return_1d']
    ...
    print("🔮 Preparando predicciones de test...")
    ...
    output_file = 'hull_tactical_predictions.parquet'
    submission_df.to_parquet(output_file, index=False)
    print(f"💾 Predicciones guardadas en: {output_file}")
    
    # También guardar como CSV para verificación
    csv_file = 'hull_tactical_predictions.csv'
    submission_df.to_csv(csv_file, index=False)
    print(f"💾 Predicciones también guardadas en: {csv_file}")
...
if __name__ == "__main__":
    try:
        submission_df, output_file = generate_predictions()
